...
TEST(solve, 4)
{
    ...
    data[0] = 1.0;
    data[1] = -1.0;
    auto result = solve(1, 2, 0.999999999999999999999);
    ASSERT_EQ(result.size(), 2);
    ASSERT_DOUBLE_EQ(result[0], -1);
    ASSERT_DOUBLE_EQ(result[1], -1);
}

TEST(solve, 5) // a не может быть равен 0
{
...
TEST(solve, 10)
{
    ...
    ASSERT_EQ(solve(std::numeric_limits<double>::infinity(), 0, 0).size(),  0);
    ASSERT_EQ(solve(-std::numeric_limits<double>::infinity(), 0, 0).size(), 0);
}
...
TEST(solve, 11)
{
    ...
    ASSERT_EQ(solve(0, std::numeric_limits<double>::infinity(),  0).size(), 0);
    ASSERT_EQ(solve(0, -std::numeric_limits<double>::infinity(), 0).size(), 0);
}
...
TEST(solve, 12)
{
    ...
    ASSERT_EQ(solve(0, 0, std::numeric_limits<double>::infinity()).size(),  0);
    ASSERT_EQ(solve(0, 0, -std::numeric_limits<double>::infinity()).size(), 0);
}
